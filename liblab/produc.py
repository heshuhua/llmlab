...
def create_it_support_kb(excel_path, faiss_index_path="it_support_faiss_index"):
    """
    从 Excel 创建 IT 支持知识库的 FAISS 向量数据库，并包含元数据。
    """
    try:
        print(f"正在读取 IT 支持知识库 Excel 文件: {excel_path}...")
        ...
        for index, row in df.iterrows():
            ...
            page_content = str(row['答案']) # 确保是字符串类型
            ...
            metadata = {
                ...
                "last_updated": str(row['最后更新']) # 保持为字符串或转换为日期对象
            }
        ...
        print(f"错误: Excel 文件中缺少列。请确保包含 'ID', '问题', '答案', '产品类别', '最后更新' 列。缺失列: {ke}")
    except Exception as e:
        print(f"发生了一个意外错误: {e}")

if __name__ == "__main__":
    excel_file = "it_support_kb.xlsx"
